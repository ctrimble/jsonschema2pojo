...
public class IncludeAccessorsPropertiesIT {
    ...
    public void onlyHasPublicInstanceFieldsWhenFalse() throws ClassNotFoundException, SecurityException, NoSuchMethodException, NoSuchFieldException {
        ...
        assertThat("only public instance fields exist", generatedType.getDeclaredFields(), everyItemInArray(anyOf(ReflectMatchers.modifiers(Modifier.STATIC), fieldWhitelist(), ReflectMatchers.modifiers(Modifier.PUBLIC))));
    }
    ...
    public void noPublicInstanceFieldsWhenTrue() throws ClassNotFoundException, SecurityException, NoSuchMethodException, NoSuchFieldException {
        ...
        assertThat("only public instance fields exist", generatedType.getDeclaredFields(), everyItemInArray(anyOf(not(ReflectMatchers.modifiers(Modifier.PUBLIC)), fieldWhitelist())));
    }
    ...
    private static <M extends Member> Matcher<M> methodWhitelist() {
        return ReflectMatchers.memberName(isIn(newArrayList(AdditionalPropertiesRule.SETTER_NAME, AdditionalPropertiesRule.GETTER_NAME, AdditionalPropertiesRule.LEGACY_GETTER_NAME)));
    }

    private static <M extends Member> Matcher<M> fieldWhitelist() {
        return ReflectMatchers.memberName(isIn(newArrayList(AdditionalPropertiesRule.FIELD_NAME)));
    }

    private static <M extends Member> Matcher<M> fieldGetterOrSetter() {
        return ReflectMatchers.memberName(anyOf(startsWith("get"), startsWith("set")));
    }
}
